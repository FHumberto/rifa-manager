...
{
    public async Task<IReadOnlyList<PesquisarParticipantesResponse>> Execute(PesquisarParticipantesRequest request)
    {
        ...
        IReadOnlyList<Participante> participantes = await participanteRepository.SearchAsync(request.Nome, request.Telefone, request.NumeroBilhete, request.StatusPagamento);
